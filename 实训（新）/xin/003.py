...
def plot_channels_created_per_year(df):
    ...
    ax.bar(channels_in_year['Year'], channels_in_year['Created Channels'], color='#32A645', label='Number of Channels (Bar)')
    ax.plot(channels_in_year['Year'], channels_in_year['Created Channels'], color='#004DFF', marker='o', label='Number of Channels (Line)')
...
def plot_top_youtube_channels(df, column, title, unit_label, conversion_factor, sort_column='subscribers', top_n=10):
    ...
    bar_cols = df.loc[:top_n - 1, ['Youtuber', sort_column]].sort_values(sort_column, ascending=True)
    bar_cols[f'{column} ({unit_label})'] = (bar_cols[sort_column] / conversion_factor).astype(int)

    fig = plt.figure(figsize=(10, 6))
    bars = plt.barh(bar_cols['Youtuber'], bar_cols[f'{column} ({unit_label})'], color=sns.color_palette('hls', len(bar_cols)))
...
def visualize_data(df):
    ...
    plot_relationship_scatter(df, 'subscribers', 'video views', 'Relationship Between Number of Subscribers & Total Video Views',
                              'Subscribers (Millions)', 'Total Video Views (Billions)')
    plot_relationship_scatter(df, 'video views', 'highest_yearly_earnings', 'Relationship Between Total Video Views & Highest Yearly Earning',
                              'Total Video Views (Billions)', 'Highest Yearly Earning (Millions)')
    plot_relationship_scatter(df, 'video views', 'uploads', 'Relationship Between Total Video Views & Total Uploads',
                              'Total Video Views (Billions)', 'Total Video Views (Billions)')
